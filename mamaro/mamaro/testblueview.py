...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        loadUi(os.getcwd()+'/mamaro/GUI/bluetooth.ui', self)
        ##set fullscreen
        #self.showFullScreen()
        ##fix size
        ...
        #_thread.start_new_thread( self.topcontent, ("Thread-1",5, ) )
        ...
        dc = MyDynamicMplCanvas(self.main_widget, width=17, height=3, dpi=100)
        #l.addWidget(sc)

        self.canvas = dc
        self.canvas.setParent(self)

        self.canvas.move(100,480)
        ...
        image_profile = QtGui.QImage(image_path) #QImage object
        self.weighticon.setPixmap(QtGui.QPixmap.fromImage(image_profile))
        self.weighticon.setScaledContents(True)
        self.weighticon.setGeometry ( ((screenw/2)-400)-(babyinfosize/2)+20, 250, babyinfosize, babyinfosize)
        #self.weighticon.setCursor(Qt.PointingHandCursor)

        image_path=os.getcwd()+"/mamaro/style/img/height.png"
        image_profile = QtGui.QImage(image_path) #QImage object
        self.heighticon.setPixmap(QtGui.QPixmap.fromImage(image_profile))
        self.heighticon.setScaledContents(True)
        self.heighticon.setGeometry ( (screenw/2)-(babyinfosize/2)+20, 250, babyinfosize, babyinfosize)

        image_path=os.getcwd()+"/mamaro/style/img/temperature.png"
        image_profile = QtGui.QImage(image_path) #QImage object
        self.tempicon.setPixmap(QtGui.QPixmap.fromImage(image_profile))
        self.tempicon.setScaledContents(True)
        self.tempicon.setGeometry ( ((screenw/2)+400)-(babyinfosize/2)+20, 250, babyinfosize, babyinfosize)

        self.weightlabel.setText("0 kg")
        self.weightlabel.setFont(QtGui.QFont("Times", 40))
        self.weightlabel.setStyleSheet('color:rgb(117, 114, 106 )')
        self.weightlabel.setGeometry ( ((screenw/2)-400)-(babyinfosize/2)+20, 420, 400, 400)
        self.weightlabel.setAlignment(QtCore.Qt.AlignTop)

        self.heightlabel.setText("0 cm")
        self.heightlabel.setFont(QtGui.QFont("Times", 40))
        self.heightlabel.setStyleSheet('color:rgb(117, 114, 106 )')
        self.heightlabel.setGeometry ( (screenw/2)-(babyinfosize/2)+20, 420, 400, 400)
        self.heightlabel.setAlignment(QtCore.Qt.AlignTop)

        self.templabel.setText("0 °C")
        self.templabel.setFont(QtGui.QFont("Times", 40))
        self.templabel.setStyleSheet('color:rgb(117, 114, 106 )')
        self.templabel.setGeometry ( ((screenw/2)+400)-(babyinfosize/2)+20, 420, 400, 400)
        self.templabel.setAlignment(QtCore.Qt.AlignTop)

        self.hrlabel.setText("0 bpm")
        self.hrlabel.setFont(QtGui.QFont("Times", 40))
        self.hrlabel.setStyleSheet('color:rgb(117, 114, 106 )')
        self.hrlabel.setGeometry ( (screenw/2)-(babyinfosize/2)+20, 800, 400, 400)
        self.hrlabel.setAlignment(QtCore.Qt.AlignTop)

        image_path=os.getcwd()+"/mamaro/style/img/blueoff.png"
        image_profile = QtGui.QImage(image_path) #QImage object
        self.bluetoothstate.setPixmap(QtGui.QPixmap.fromImage(image_profile))
        self.bluetoothstate.setScaledContents(True)
        self.bluetoothstate.setGeometry ( 120, 900, 60, 60)

        ####baby information####

        ####middle content####
        # image_path=os.getcwd()+"/mamaro/style/img/pleaselock.png"
        # image_profile = QtGui.QImage(image_path) #QImage object
        # self.middleleft.setPixmap(QtGui.QPixmap.fromImage(image_profile))
        # self.middleleft.setScaledContents(True)
        # self.middleleft.setGeometry ( 200, 200, 700, 700)
        # self.middleleft.setCursor(Qt.PointingHandCursor)



        # self.middleleft.setText("Weight:"+"\n"+"Temperature:"+"\n"+"Heartrate:"+"\n"+"Height:"+"\n")
        # self.middleleft.setFont(QtGui.QFont("Times", 40))
        # self.middleleft.setStyleSheet('color:rgb(117, 114, 106 )')
        # self.middleleft.setGeometry ( 200, 400, 700, 400)
        # self.middleleft.setAlignment(QtCore.Qt.AlignTop)
        #
        #
        # #self.middleright.setText("扉の鍵を閉めてください。")
        # self.middleright.setFont(QtGui.QFont("Times", 40))
        # self.middleright.setStyleSheet('color:rgb(117, 114, 106 )')
        # self.middleright.setGeometry ( 1100, 400, 700, 400)
        # self.middleright.setAlignment(QtCore.Qt.AlignTop)



        # self.middletitle.setText("扉の鍵を閉めてください。")
        # self.middletitle.setFont(QtGui.QFont("Times", 20))
        ...
        # self.textEdit_2.textChanged.connect(self.setzero)
    def counter(self):
        total=60*20
        checkreadfile=True
        ...
        try:
            with open(os.getcwd()+"/mamaro/beacon_value.text", "r+") as f:
                ...
                if hh=="0":
                    ...
                    self.bluetoothstate.setPixmap(QtGui.QPixmap.fromImage(image_profile))
                ret+=ww+" kg\n"
                self.weightlabel.setText(ww+" kg")
                ret+=tem+" °C\n"
                self.templabel.setText(tem+" °C")
                ret+=hb+" bpm\n"
                self.hrlabel.setText(hb+" bpm")
                ret+=hh+" cm\n"
                self.heightlabel.setText(hh+" cm")
                y.append(float(hb))
                del y[0]
    ...
    def Convert(self):
        ...
        os.system("kill `pgrep -f window.py`")
    ...
    def getdata(self, event):
        #get data
        self.contentlist.append(company(os.getcwd()+"/style/img/_icon.png","company name","cool","website url"))
        try:
...
if __name__ == "__main__":
    ...
    w = MainWindow()
    w.setWindowTitle('mamaro')
    QtWidgets.qApp.setApplicationDisplayName('bleview')
    w.show()
    sys.exit(app.exec_())
